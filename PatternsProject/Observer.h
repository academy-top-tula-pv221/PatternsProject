...
class Broker : public IObserver
{
	...
	IObservable* market;
public:
	Broker(string name, IObservable* market) : name{ name }, market{ market } 
	{
...
class Bank : public IObserver
{
	...
	IObservable* market;
public:
	Bank(string name, IObservable* market) : name{ name }, market{ market } 
	{
		market->AddObserver(this);
	}
	string& Name() { return name; }
	...
	{
		if (info->Euro() > 50)
			cout << "Bank " << name << "saled usd at " << info->Euro() << "\n";
		else
			cout << "Bank " << name << "byed usd at " << info->Euro() << "\n";
	}
